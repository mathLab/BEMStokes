  ...
  for (unsigned int degree=1; degree<=max_degree; degree++)
    {
      ...
      bem_problem_3d.fe_stokes =  SP(FETools::get_fe_by_name<dim-1,dim>(fe_name));
      bem_problem_3d.fe_map = SP(FETools::get_fe_by_name<dim-1,dim>(fe_name));
      ...
      for (unsigned int cycle=0; cycle<ncycles; ++cycle)
        {
          ...
          Vector<double> t1_diff(trace_1_vector_difference),
                 ...
                 t0(G_velocities);
          eh.error_from_exact(*bem_problem_3d.mappingeul, bem_problem_3d.dh_stokes, t1_diff, ZeroFunction<3, double> (3),0);
