...
int main (int argc, char **argv)
{
  ...
  std::cout<<"Test on the boundary in 3D for the Single Layer"<<std::endl;
  ...
  std::cout<<"Boundary Point at "<< val_points[0]<<" , expected "<< 0.*dummy_vel(0)<<" "<< 0.*dummy_vel(1)<<" "<<0.*dummy_vel(2)<<std::endl;
  double foo = 0;
  for (unsigned int k=0; k<dim; ++k)
    foo += std::pow(eval_vel(k)-0.*dummy_vel(bem_problem_3d.dh_stokes.n_dofs()/dim*k),2);
