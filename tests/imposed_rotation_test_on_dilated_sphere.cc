...
int main (int argc, char **argv)
{
  ...
  double torque=1.;
  double L=10.;
  double exact_omega=torque/8./numbers::PI/(L*L*L);
  // tol/=(L*L*L);
  unsigned int ncycles = 4;
  unsigned int max_degree = 1;
  std::cout<<"Test for a rotation of a sphere in the space"<<std::endl;
  ...
  bem_problem_3d.force_pole="Baricenter";
  ...
  for (auto i : bem_problem_3d.euler_vec.locally_owned_elements())
    bem_problem_3d.euler_vec[i]=bem_problem_3d.euler_vec[i]*L+34.913639;
  ...
  for (unsigned int i=3; i<6; ++i)
    {
      ...
      if (std::fabs(bem_problem_3d.rigid_velocities[i]-exact_omega)/std::fabs(exact_omega)>tol)
        {
          std::cout<<"ERROR : "<<bem_problem_3d.rigid_velocities[i]<<" Vs "<<exact_omega<<" "<<std::fabs(bem_problem_3d.rigid_velocities[i]-exact_omega)/std::fabs(exact_omega)<<" "<<tol<<std::endl;
        }
      else
        {
          std::cout<<" OK OMEGA_"<<i<<std::endl;
        }
