  ...
  for (unsigned int i=3; i<6; ++i)
    {
      ...
      if (std::fabs(bem_problem_3d.rigid_velocities[i]-exact_omega)>tol)
        {
          std::cout<<"ERROR : "<<bem_problem_3d.rigid_velocities[i]<<" Vs "<<exact_omega<<" "<<std::fabs(bem_problem_3d.rigid_velocities[i]-exact_omega)<<std::endl;
        }
      else
        {
