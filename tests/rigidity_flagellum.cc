...
int main (int argc, char **argv)
{
  ...
  bem_problem_3d.input_grid_base_name="flagellum_";
  ...
  for (unsigned int i=0; i<6; ++i)
    {
      ...
      bem_problem_3d.output_save_stokes_results(i);
      for (unsigned int j=0; j<6; ++j)
        {
          if (j!=i)
            {
              double ratio=std::fabs(bem_problem_3d.baricenter_rigid_velocities[j]/bem_problem_3d.baricenter_rigid_velocities[i]);
              if (ratio < tol)
                std::cout<<"OK"<<std::endl;
              else
                std::cout<<ratio<<" "<<bem_problem_3d.baricenter_rigid_velocities[j]<<" "<<bem_problem_3d.baricenter_rigid_velocities[i]<<std::endl;
            }
        }
