...
int main (int argc, char **argv)
{
  ...
  bem_problem_3d.pcout<<"Testing null stresses on the simmetry plane"<<std::endl;
  ...
  bem_problem_3d.approximate_velocity_gradient(test_points,loc_velocities,loc_stokes_forces,h_grad,val_gradients);

  // for(unsigned p = 0; p<test_points.size(); ++p)
  // {
  //   std::cout<<"point : "<<test_points[p]<<" gradient : ";
  //   std::cout<<val_gradients[p]<<" ";
  //   std::cout<<std::endl;
  // }
  // for(unsigned p = 0; p<test_points.size(); ++p)
  // {
  //   std::cout<<"point : "<<test_points[p]<<" gradient check : ";
  //   std::cout<<" du/dy = "<<val_gradients[p][0][1]<<" ";
  //   std::cout<<", dv/dy = "<<val_gradients[p][2][1]<<" ";
  //   std::cout<<", dv/dx = "<<val_gradients[p][1][0]<<" ";
  //   std::cout<<" dv/dz = "<<val_gradients[p][1][2]<<" ";
  //   std::cout<<std::endl;
  // }
  ...
  for (unsigned p = 0; p<test_points.size(); ++p)
    {
      ...
      double foo = std::abs(val_gradients[p][0][1]);
      if (foo>tol)
        bem_problem_3d.pcout<<"ERROR, index p : "<<p<<" : du/dy = "<<val_gradients[p][0][1]<<" , instead of : "<<0<<std::endl;
      else
        bem_problem_3d.pcout<<"OK"<<std::endl;
      foo = std::abs(val_gradients[p][2][1]);
      if (foo>tol)
        bem_problem_3d.pcout<<"ERROR, index p : "<<p<<" : dw/dy = "<<val_gradients[p][0][1]<<" , instead of : "<<0<<std::endl;
      else
        bem_problem_3d.pcout<<"OK"<<std::endl;
      foo = std::abs(val_gradients[p][1][0]);
      if (foo>tol)
        bem_problem_3d.pcout<<"ERROR, index p : "<<p<<" : dv/dx = "<<val_gradients[p][0][1]<<" , instead of : "<<0<<std::endl;
      else
        bem_problem_3d.pcout<<"OK"<<std::endl;
      foo = std::abs(val_gradients[p][1][2]);
      if (foo>tol)
        bem_problem_3d.pcout<<"ERROR, index p : "<<p<<" : dv/dz = "<<val_gradients[p][0][1]<<" , instead of : "<<0<<std::endl;
      else
        bem_problem_3d.pcout<<"OK"<<std::endl;
