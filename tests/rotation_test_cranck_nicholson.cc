      ...
      for (unsigned int j=0; j<(int)(1)/dt; ++j)
        {
          ...
          omega[(i+2)%dim]=(cos(2*numbers::PI*(j)/((int)(1)/dt))*(2*numbers::PI)+cos(2*numbers::PI*(j)/((int)(1)/dt))*(2*numbers::PI))/2;
          bem_problem_3d.update_rotation_matrix(rotation_matrix,omega,dt,false,0.5);
          // rotation_matrix.print_formatted(std::cout);
