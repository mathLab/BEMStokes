...
int main (int argc, char **argv)
{
  ...
  bem_problem_3d.input_grid_base_name="sphere_rotation_";
  ...
  double exact_omega=-2.*numbers::PI/120./bem_problem_3d.time_step;
  ...
  // bem_problem_3d.this_cpu_set.print(std::cout);
  // bem_problem_3d.body_cpu_set.print(std::cout);
  // std::cout<<bem_problem_3d.assemble_scaling<<std::endl;
  // TrilinosWrappers::MPI::Vector dummy_1(bem_problem_3d.this_cpu_set, bem_problem_3d.mpi_communicator),
  //                               dummy_2(bem_problem_3d.this_cpu_set, bem_problem_3d.mpi_communicator),
  //                               dummy_3(bem_problem_3d.this_cpu_set, bem_problem_3d.mpi_communicator);
  //
  // dummy_1.sadd(0.,bem_problem_3d.rigid_velocities[3],bem_problem_3d.N_rigid[3]);
  // bem_problem_3d.K_matrix.vmult(dummy_2, dummy_1);
  // bem_problem_3d.K_matrix.vmult(dummy_3, bem_problem_3d.shape_velocities);
  // dummy_2.print(std::cout);
  // dummy_3.print(std::cout);
  // bem_problem_3d.stokes_forces.print(std::cout);
  for (unsigned int i=0; i<3; ++i)
    {
      if (std::fabs(bem_problem_3d.rigid_velocities[i])<=tol)
        std::cout<<"OK rigid translation "<<i<<std::endl;
      else
        std::cout<<"ERROR on rigid traslation "<<i<<" : "<<bem_problem_3d.rigid_velocities[i]<<std::endl;
    }
  if (std::fabs(bem_problem_3d.rigid_velocities[3]+exact_omega)/std::fabs(exact_omega)<=tol)
    std::cout<<"OK rigid rotation "<<3<<std::endl;
  else
    std::cout<<"ERROR on rigid rotation "<<3<<" : "<<bem_problem_3d.rigid_velocities[3]<<" , "<<exact_omega<<" , "<<std::fabs(bem_problem_3d.rigid_velocities[3]+exact_omega)/std::fabs(exact_omega)<<std::endl;

  for (unsigned int i=4; i<6; ++i)
    {
      if (std::fabs(bem_problem_3d.rigid_velocities[i])<=tol)
        std::cout<<"OK rigid rotation "<<i<<std::endl;
      else
        std::cout<<"ERROR on rigid rotation "<<i<<" : "<<bem_problem_3d.rigid_velocities[i]<<std::endl;
    }
